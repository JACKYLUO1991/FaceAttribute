...
def facenet_resnet(nb_class=101, embdding=256, is_train=False, weights=None):
    ...
    # used for training
    if is_train:
        assert weights is not None, "weights not find..."
        base_model.load_weights(weights)

    x = base_model.get_layer('Dropout').output

    fc_a = Dense(embdding, activation='relu', name='fc_a')(x)
    pred_a = Dense(nb_class, activation='softmax', name='pred_a',
    ...
    pred_e = Dense(7, activation='softmax', name='pred_e',
                   kernel_initializer="he_normal")(fc_e)

    model = Model(base_model.input, [pred_g, pred_a, pred_e])
